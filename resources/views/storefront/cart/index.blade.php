                ...
                <div class="space-y-5">@foreach($cart->items as $item)
                    <article class="grid gap-5 rounded-[2rem] border-b-4 border-rose/30 bg-white p-5 last:border-b-0 sm:grid-cols-[11rem_1fr] sm:border-b sm:border-rose/25">
                        <div class="flex h-auto w-full max-w-full items-center justify-center overflow-hidden rounded-2xl sm:h-52 sm:w-44 sm:p-2" @if($item->composedDesign) style="background-color: {{ $item->composedDesign->previewSurfaceColour() }}" @endif>
                            <img src="{{ $item->composedDesign ? route('artwork.designs', [$item->artworkSession->public_id, $item->composedDesign]) : route('artwork.assets', [$item->artworkSession->public_id, $item->generationAsset]) }}" alt="Approved {{ $item->artwork_style_name }} design" class="h-auto w-full object-contain sm:h-full">
                        </div>
                        ...
                        <div><h2 class="font-display text-2xl">{{ $item->product_name }}</h2><p class="mt-1 text-sm text-muted">{{ $item->variant_name }} · {{ $item->artwork_style_name }}</p>
                            ...
                            <div class="relative -top-[50px] -mb-[50px] sm:-top-[70px] sm:-mb-[70px]">
                                <div class="mt-5 flex flex-wrap items-end justify-between gap-4">
                                    <form method="POST" action="{{ route('cart.quantity', $item) }}" class="flex items-center gap-2 lg:relative lg:top-[50px]">@csrf @method('PATCH')<label for="quantity-{{ $item->id }}" class="text-sm font-bold">Quantity</label><select id="quantity-{{ $item->id }}" name="quantity" onchange="this.form.submit()" class="rounded-xl border border-rose/30 bg-white px-3 py-2">@for($n=1;$n<=config('commerce.max_quantity');$n++)<option value="{{ $n }}" @selected($item->quantity===$n)>{{ $n }}</option>@endfor</select></form>
