...
<section class="shell py-12 sm:py-20"><div class="mx-auto max-w-5xl">
    ...
    <div class="mt-10 rounded-[2rem] bg-white p-7 sm:p-9"><div class="flex items-center justify-between gap-4"><h2 class="font-display text-3xl">Your orders</h2>@if($orders->isNotEmpty())<a class="font-bold text-coral" href="{{ route('account.orders.index') }}">View all orders</a>@endif</div>
        ...
        @else<div class="mt-6 divide-y divide-rose/20">@foreach($orders as $order)<a href="{{ route('account.orders.show', $order->number) }}" class="grid gap-2 py-5 first:pt-0 sm:grid-cols-[1fr_auto] sm:items-center"><div><strong>{{ $order->number }}</strong><p class="mt-1 text-sm text-muted">{{ ($order->placed_at ?? $order->created_at)->format('j F Y') }} · {{ $order->status->customerLabel() }} · {{ $order->items_count }} {{ Str::plural('item', $order->items_count) }}</p></div><strong>£{{ number_format(($order->total_minor ?? 0) / 100, 2) }}</strong></a>@endforeach</div>@endif
    </div>
</div></section>
