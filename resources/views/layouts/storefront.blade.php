...
<header class="sticky top-0 z-40 border-b border-rose/20 bg-cream/95 backdrop-blur" x-data="{mobileMenu:false,mobileAccountOpen:false,mobileOpenMenu:null,desktopOpenMenu:null,searchOpen:false,searchQuery:@js((string) request('q', '')),searchProducts:@js($searchProducts),filteredProducts(){const stop=['for','the','a','an','and','or','to','of','in','on','with','my','your'];const raw=this.searchQuery.toLowerCase().split(/\s+/).filter(Boolean);const meaningful=raw.filter(w=>!stop.includes(w));const terms=meaningful.length?meaningful:raw;return terms.length===0?this.searchProducts:this.searchProducts.filter(product=>{const name=product.name.toLowerCase();return terms.every(w=>name.includes(w))})}}">
    <div class="shell py-4 lg:grid lg:grid-cols-[auto_minmax(240px,1fr)_auto] lg:items-center lg:gap-8">
        <div class="flex items-center justify-between pl-[50px]">
            ...
            <div class="flex items-center gap-4 lg:hidden">
                <div class="relative" @click.outside="mobileAccountOpen=false">
                    <button type="button" class="cursor-pointer rounded-full p-1 {{ request()->routeIs('account.*') ? 'text-coral' : 'text-ink' }}" @click="mobileAccountOpen=!mobileAccountOpen" :aria-expanded="mobileAccountOpen" aria-label="My Account"><i data-lucide="{{ auth()->check() ? 'lock-keyhole' : 'user-round' }}" class="h-5 w-5" aria-hidden="true"></i></button>
        ...
        <nav aria-label="Main navigation" class="hidden items-center gap-6 whitespace-nowrap text-sm font-semibold lg:flex">
            ...
            <div class="relative" @click.outside="desktopOpenMenu=null">
                <a class="nav-link inline-flex cursor-pointer items-center gap-1.5 {{ request()->routeIs('account.*') ? 'text-coral' : '' }}" href="@auth{{ route('account.index') }}@else{{ route('login') }}@endauth" @mouseenter="desktopOpenMenu='account'"><i data-lucide="{{ auth()->check() ? 'lock-keyhole' : 'user-round' }}" class="h-4 w-4" aria-hidden="true"></i><span>@auth My Account @else Sign in @endauth</span><i data-lucide="chevron-down" class="h-3.5 w-3.5 cursor-pointer" aria-hidden="true" @click.prevent="desktopOpenMenu = desktopOpenMenu==='account' ? null : 'account'"></i></a>
