...
class RequestArtworkGeneration
{
    ...
    public function handle(ArtworkSession $session, bool $regeneration = false, bool $dispatch = true)
    {
        return DB::transaction(function () use ($session, $regeneration, $dispatch) {
            ...
            $activeGeneration = $session->generations()
                ->whereIn('status', [GenerationStatus::Pending->value, GenerationStatus::Queued->value, GenerationStatus::Processing->value])
                ->latest('created_at')
                ->first();
            if ($activeGeneration) {
            ...
            $session->update(['current_generation_id' => $generation->id, 'status' => ArtworkSessionStatus::Generating, 'processing_stage' => ArtworkProcessingStage::CreatingIllustration, 'expires_at' => now()->addDays(config('artwork.retention_days'))]);
