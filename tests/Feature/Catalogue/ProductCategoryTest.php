...
class ProductCategoryTest extends TestCase
{
    ...
    public function test_category_pages_are_active_only_and_hide_inactive_products(): void
    {
        ...
        $category->products()->attach([$active->id, $inactive->id]);

        $this->get(route('categories.show', $category))->assertOk()
            ->assertSee('School &amp; Lunch', false)->assertSee('Active Bottle')->assertDontSee('Hidden Bottle')
    ...
    public function test_seeded_bottle_belongs_to_both_categories_without_changing_variants(): void
    {
        Storage::fake('public');
        ...
        $this->get(route('sitemap.xml'))->assertOk()
            ->assertSee('/collections/school-lunch')->assertSee('/collections/kids-drinkware')
    ...
    {
        $visible = ProductCategory::factory()->create(['name' => 'School & Lunch', 'slug' => 'school-lunch']);
        ProductCategory::factory()->create(['name' => 'Empty Category', 'slug' => 'empty']);
        ...
        $this->get(route('products.index'))->assertOk()->assertSee('Shop by category')
            ->assertSee('School &amp; Lunch', false)->assertDontSee('Empty Category')->assertDontSee('Hidden Category')
        ...
        $this->get(route('products.show', $product->slug))->assertOk()
            ->assertSee('Categories:')->assertSee('School &amp; Lunch', false)->assertDontSee('Hidden Category')
